// THIS FILE IS AUTO GENERATED BY GK-CLI DO NOT EDIT!!
package endpoint

import (
	endpoint "github.com/go-kit/kit/endpoint"
	service "github.com/justcy/uc/notificator/pkg/service"
)

// Endpoints collects all of the endpoints that compose a profile service. It's
// meant to be used as a helper struct, to collect all of the endpoints into a
// single parameter.
type Endpoints struct {
	SendSmsEndpoint   endpoint.Endpoint
	CheckSmsEndpoint  endpoint.Endpoint
	SendEmailEndpoint endpoint.Endpoint
}

// New returns a Endpoints struct that wraps the provided service, and wires in all of the
// expected endpoint middlewares
func New(s service.NotificatorService, mdw map[string][]endpoint.Middleware) Endpoints {
	eps := Endpoints{
		CheckSmsEndpoint:  MakeCheckSmsEndpoint(s),
		SendEmailEndpoint: MakeSendEmailEndpoint(s),
		SendSmsEndpoint:   MakeSendSmsEndpoint(s),
	}
	for _, m := range mdw["SendSms"] {
		eps.SendSmsEndpoint = m(eps.SendSmsEndpoint)
	}
	for _, m := range mdw["CheckSms"] {
		eps.CheckSmsEndpoint = m(eps.CheckSmsEndpoint)
	}
	for _, m := range mdw["SendEmail"] {
		eps.SendEmailEndpoint = m(eps.SendEmailEndpoint)
	}
	return eps
}
